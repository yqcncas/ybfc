<template>
	<view :class="{ text: styleType === 'text' }" :style="{ borderColor: styleType === 'text' ? '' : '' }" class="segmented-control">
		<view v-for="(item, index) in values" :class="[{ text: styleType === 'text' }, { active: index === currentIndex }]" :key="index" :style="{
        color:
          index === currentIndex
            ? activeColor
              : '#fff',
        backgroundColor: index === currentIndex && styleType === 'button' ? backgroundColor : ''
