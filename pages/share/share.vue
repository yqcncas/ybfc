...
<script>
	...
	export default{
		...
		methods:{
			...
			async getPosterInfo(){
				if(this.type == 1){
					let res = await this.$fetch(this.$api.houseInfo,{houseId:this.id},'GET')
					this.title = res.data.oldHouse.houseName
					if(res.data.oldHouse.picUrl){
						let picUrls = res.data.oldHouse.picUrl.split(',')
						if(picUrls[0]){
							this.img = picUrls[0]
					...
					if(res.data.oldHouse.rental){
						this.price = res.data.oldHouse.rental+'元/月'
					}
					if(res.data.oldHouse.room1){
						this.homeType = res.data.oldHouse.room1+'室'+res.data.oldHouse.room2+'厅'+res.data.oldHouse.room3+'卫'
					}
					if(res.data.oldHouse.acreage){
						this.acreage = res.data.oldHouse.acreage+'m²'
					}
					if(this.isAdmin == true){
						this.path = 'pages/oldHouse/oldHouse?houseId='+this.id+'&houseType='+this.type+'&userId='+this.userId+'&share=1'
						this.page = 'pages/oldHouse/oldHouse'
						this.scene = this.id+'&'+this.type+'&'+this.userId
					}else{
						this.path = 'pages/oldHouse/oldHouse?houseId='+this.id+'&houseType='+this.type+'&share=1'
						this.page = 'pages/oldHouse/oldHouse'
					...
					if(res.data.oldHouse.price){
						this.price = res.data.oldHouse.price+'万'
					}
					if(res.data.oldHouse.room1){
						this.homeType = res.data.oldHouse.room1+'室'+res.data.oldHouse.room2+'厅'+res.data.oldHouse.room3+'卫'
					}
